...
Usage:
    python -m pfs.etl.pipeline ingest NVDA --years 5
    python -m pfs.etl.pipeline ingest NVDA --years 5 --quarterly
    python -m pfs.etl.pipeline ingest-batch NVDA,AAPL,MSFT --years 5
    python -m pfs.etl.pipeline ingest-sp500
    python -m pfs.etl.pipeline sync-prices
    python -m pfs.etl.pipeline sync-prices --tickers NVDA,AAPL
"""
...
from pfs.etl import sec_client, xbrl_parser
from pfs.etl.price_client import get_daily_prices
from pfs.etl.validation import validate_financials, validate_price_data
from pfs.etl.yfinance_client import get_market_data, get_stock_info, get_stock_splits
...
def ingest_company(
    ...
    try:
        ...
        from pfs.etl.data_fallback import (
        ...
        try:
            from pfs.etl.section_extractor import extract_and_save
        ...
        try:
            from pfs.etl.yfinance_client import get_key_financials
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m pfs.etl.pipeline",
