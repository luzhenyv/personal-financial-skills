...
4. **Store the split history** in `data/artifacts/{TICKER}/profile/stock_splits.json` for auditability
...
if not splits.empty:
    ...
    out = Path(f"data/artifacts/{ticker}/profile/stock_splits.json")
